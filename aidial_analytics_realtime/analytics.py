...
from aidial_analytics_realtime.anthropic_messages_api import (
    get_anthropic_messages_request_contents,
    get_anthropic_messages_response_contents,
)
...
def _get_number_anthropic_messages(request: dict | None) -> int:
    if request is None:
        return 0
    return len(request.get("messages") or [])


async def make_anthropic_messages_point(
    *,
    deployment: str,
    model: str,
    parent_deployment: str | None,
    ...
    if chat_id:
        request_contents = get_anthropic_messages_request_contents(request)
        response_contents = get_anthropic_messages_response_contents(response)
    ...
    return (
        ...
        .field(
            "number_request_messages",
            _get_number_anthropic_messages(request),
        )
        .field("price", float(price))
        .field("deployment_price", float(deployment_price))
